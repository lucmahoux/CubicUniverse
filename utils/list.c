...
cubList* cub_utils_list(size_t memb_size, cubReallocType type,
                        size_t capacity) {
    ...
    list->len = 0;
    list->realloc_fun = type == CUB_REALLOC_DOUBLE
...
void cub_utils_list_check_double_capacity(cubList* list) {
    if (list->len >= list->capacity) {
        ...
        if (!list->data)
            errx(1, "cub_utis_list_check_double_capacity: realloc failed!");
    }
}

void cub_utils_list_check_plus_one_realloc(cubList* list) {
    if (list->len >= list->capacity) {
        ++list->capacity;
        list->data = realloc(list->data, list->memb_size * list->capacity);
        if (!list->data)
            errx(1, "cub_utis_list_check_plus_on: realloc failed!");
    }
}
...
void cub_utils_list_remove_ordered(cubList* list, size_t i) {
    if (i >= list->len)
        printf("cub_utils_list_remove_sorted: index {%lu} out of range!\n", i);
    else {
        for (size_t k = i + 1; k < list->len; ++k)
            memcpy( cub_utils_list_get(list, k - 1),
                    cub_utils_list_get(list, k),
                    list->memb_size );
        --list->len;
    }
}
